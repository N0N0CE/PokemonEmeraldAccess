...
function Drawing.drawBg()
	local y_start = 190
	local inc = 10
	local i = 0
	local x = 0

	local function place_draw(str)
		if i == 16 then 
			x = 100
			y_start = 190
			i = 0
		end	
		Drawing.drawText(x, y_start+i*inc, str)	
		i = i + 1	
	end

	for ii=0,3,1 do
		place_draw(" Bg" .. ii .. ":")
		for k, v in pairs(pbgr[ii]) do
			place_draw(k .. "=" .. v)
		
		end
		--place_draw("scroll=" .. bgs[ii])
	end
	
	for ii=0,3,1 do
		place_draw(bizstring.binary(bgr[ii]))
	end
	
	
end

function Drawing.drawTiles()

	
end

function Drawing.drawWindows()
	local y_start = 190
	local inc = 10
	local i = 0
	local x = 0
	for k, v in pairs(frames) do
		...
		Drawing.drawText(x, y_start+i*inc, "x=" .. v.x .. ",y=" .. v.y .. ",x2=" .. v.x2 .. ",y2=" .. v.y2)	
		i = i + 1		
	end
	
end

function Drawing.drawMisc()
	local y_start = 190
	local inc = 10
	local i = 0
	local x = 5
	for k, v in pairs(misc.values) do
		if i == 16 then 
			x = 125
			y_start = 190
			i = 0
		end
		Drawing.drawText(x, y_start+i*inc, k .. "=" .. v)	
		i = i + 1
	end
